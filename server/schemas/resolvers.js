...
const resolvers = {
    Query: {
      ...
      admin: async (parent, args, context) => {
        if (context.admin) {
          const admin = await Admin.findById(context.admin._id).populate('gallery').populate('events').populate('tag');
    ...
    Mutation: {
      ...
      addGallery: async (parent, args, context) => {
        if (context.admin) {

          const gallery = await Gallery.create({ ...args, email: context.admin.email });
      ...
      addEvents: async (parent, args, context) => {
        if (context.admin) {
          ...
          const event = await Events.create({ ...args, email: context.admin.email });
      ...
      updateAdmin: async (parent, args, context) => {
        if (context.admin) {
          return await Admin.findByIdAndUpdate(context.admin._id, args, { new: true });
        }
